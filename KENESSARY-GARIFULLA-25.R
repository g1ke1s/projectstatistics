...
qq_plot_nolegend <- function(var) {
  ggplot(df, aes(sample = get(var))) +
    stat_qq(aes(color = "Sample Quantiles"), size = 1.2) +
    stat_qq_line(aes(color = "Normal Line"), size = 1, linetype = "dashed") +
    scale_color_manual(values = c("Sample Quantiles" = "blue", "Normal Line" = "red")) +
    labs(title = paste("Q-Q Plot:", var), x = "Theoretical Quantiles", y = "Sample Quantiles", color = NULL) +
    theme_minimal() +
    theme(legend.position = "none")
}
...
# Build plot list
plots <- list()
plots[[1]] <- qq_plot_with_legend(vars[1])  # first with legend
for (i in 2:length(vars)) {
  plots[[i]] <- qq_plot_nolegend(vars[i])
}
while (length(plots) < 9) {
  plots[[length(plots) + 1]] <- ggplot() + theme_void()
}

# Combine and render with shared legend
final_plot <- wrap_plots(plots, ncol = 3, guides = "collect") & theme(legend.position = "right")

# Save and display
ggsave("qqplots_with_shared_legend.png", plot = final_plot, width = 16, height = 10, dpi = 300)
print(final_plot)
...
# Save and print
ggsave("qqplots_with_sf_annotations.png", final_plot, width = 16, height = 10, dpi = 300)
print(final_plot)
